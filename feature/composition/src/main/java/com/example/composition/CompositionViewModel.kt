...
@HiltViewModel
internal class CompositionViewModel @Inject constructor(
    ...
    private fun getDiaryInfo() {
        viewModelScope.launch {
            if (isInEditMode()) {
                ...
                repository.pullDiary(ObjectId(id!!))
                    .catch { ex ->
    ...
    fun storeDiary(diary: com.example.util.models.DiaryEntry, onSuccess: () -> Unit, onFailure: (String) -> Unit) {
        viewModelScope.launch(Dispatchers.IO) {
            repository.upsertEntry(diary.apply {
    ...
    fun deleteDiary(onSuccess: () -> Unit, onFailure: (String) -> Unit) {
        viewModelScope.launch(Dispatchers.IO) {
            val result = repository.deleteDiary(diaryId = ObjectId(uiState.value.selectedDiaryEntryId!!))
